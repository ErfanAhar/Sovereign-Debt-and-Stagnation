...
function simulate(model::Model, sol::Solution; T::Int = 200_000, b0_idx::Int = 1,
    g0_idx::Int = 1, eps0_idx::Int = Int(cld(model.Ne, 2)),
    ...
    for t in 1:T
        ...
        if !in_default && sol.d[b_idx, g_idx, eps_idx]
            in_default = true
        end
        ...
        if in_default
            ...
            y_path[t] = y_base
            b_idx_next = sol.b_policy_idx[b_idx, g_idx, eps_idx]
            n_path[t] = sol.n[b_idx_next, g_idx]
            R_path[t] = sol.R[b_idx_next, g_idx]
        end
        ...
        if !default_path[t]
            ...
            if rand(rng) < model.theta
                ...
                v_repay = sol.vnd[kb_idx_next_default, g_idx_next, eps_idx_next]
                v_def = sol.vd[b_idx_next_default, g_idx_next, eps_idx_next]
                if v_repay >= v_def
                    in_default = false
    ...
    return (b = b_path, y = y_path, n = n_path, R = R_path, default = default_path,
        g_idx = g_idx_path, eps_idx = eps_idx_path)
end
