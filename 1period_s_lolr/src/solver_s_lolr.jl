    ...
    for gi in 1:Ng, si in 1:Ns
        ...
        Threads.@threads for tidx in 1:(Ne * Nl)
            ...
            for lprimej in 1:l_last
                resource_row = reshape(view(n_choice, :, lprimej) .+ n_l_vec[lprimej], 1, Nb)
                continuation_row = reshape(g_beta[gi] .* view(wE_gs, :, lprimej), 1, Nb)
                mask_row = reshape(view(allowed, :, lprimej), 1, Nb)
                c = y_cur - l_cur .+ resource_row .- bcol
                val = u.(c, γ) .+ continuation_row
                best_l = vec(maximum(ifelse.(mask_row, val, -Inf), dims = 2))
                @. best = max(best, best_l)
            end
    ...
    for gi in 1:Ng, si in 1:Ns
        ...
        Threads.@threads for tidx in 1:(Ne * Nl)
            ...
            for lprimej in 1:l_last
                ...
                c = y_cur - l_cur .+ resource_row .- bcol
                val = u.(c, γ) .+ continuation_row
                row_val, row_idx = findmax(ifelse.(mask_row, val, -Inf), dims = 2)
                row_best = vec(row_val)
                row_b = getindex.(vec(row_idx), 2)
                better = row_best .> best_val
                best_val[better] .= row_best[better]
                best_b[better] .= row_b[better]
                best_l[better] .= lprimej
            end
