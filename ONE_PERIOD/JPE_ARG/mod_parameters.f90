module mod_parameters
   ...
   ! Grids
    ...
    ! value and policies
    real(8), dimension(Nb,Ng,Ns,Nz) :: Vnd              ! value function no-default
    real(8), dimension(Nb,Ng,Ns,Nz) :: Vd               ! value function default
	real(8), dimension(Nb,Ng,Ns,Nz) :: Vndk             ! Vnd(kappa*b,g,s,z)
    real(8), dimension(Nb,Ng,Ns,Nz) :: Wv               ! Wv = max{Vnd , Vd}
	real(8), dimension(Nb,Ng,Ns,Nz) :: Wtv              ! Wv = max{Vndk, Vd}
    real(8), dimension(Nb,Ng,Ns)    :: EWv              ! EWv(b,g,s)  = sum_{g',s',z'}{Wv(b,g',s',z') p(z')p(g'|g)p(s'|s)}
    real(8), dimension(Nb,Ng,Ns)    :: EVd              ! EVd(b,g,s)  = sum_{g',s',z'}{Vd(b,g',s',z') p(z')p(g'|g)p(s'|s)}
	real(8), dimension(Nb,Ng,Ns)    :: EWtv             ! EWtv(b,g,s) = sum_{g',s',z'}{Wtv(b,g',s',z')p(z')p(g'|g)p(s'|s)}    
    
    integer, dimension(Nb,Ng,Ns,Nz) :: bopt_loc
    
	real(8), dimension(Nb,Ng,Ns,Nz) :: bpol             ! debt policy: bpol = bvec(bopt_loc)	
	real(8), dimension(Nb,Ng,Ns,Nz) :: ppol             ! probability of default policy	
	real(8), dimension(Nb,Ng,Ns,Nz) :: rpol             ! interest rate policy		
    real(8), dimension(Nb,Ng,Ns,Nz) :: dpol             ! default policy
    real(8), dimension(Nb,Ng,Ns,Nz) :: epol             ! entry policy
    
    real(8), dimension(Nb,Ng,Ns,Nz) :: Qprice           ! price of no defaulted bond
    real(8), dimension(Nb,Ng,Ns,Nz) :: Xprice           ! price of defaulted bond
    real(8), dimension(Nb,Ng,Ns)    :: EQ,EXX           ! EQ(b,g,s) = sum_{g',s',z'}{Q(b,g',s',z')p(z')p(g'|g)p(s'|s)}
    
    real(8), dimension(Nb,Ng,Ns)    :: Rsched           ! R(b',g,s) -> the "full" schedule // note that b doesn't matter given b'
	real(8), dimension(Nb,Ng,Ns)    :: rhosched         ! interest rate implied by R(b',g,s)	
    real(8), dimension(Nb,Nb,Ng,Ns) :: nsched           ! n(b',b,g,s) -> gb' = (1-dlta)*b + R(b',g,s)n(b',b,g,s)
	real(8), dimension(Nb,Ng,Ns)    :: Ednp				! expected dfault next period		
	integer, dimension(Nb,Nb,Ng,Ns) :: lsch,isch,lisch,msch
